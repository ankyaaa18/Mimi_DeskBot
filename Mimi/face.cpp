...
void faceHappy()
{
    ...
    // Freeze the face while happy: no auto-blink flicker, no idle drift.
    // Without this, updateAutoBlink()/updateIdle() keep running in the
    // background (they don't know about expression at all) and idle
    // drift in particular would still nudge eyeX/eyeY, which visibly
    // shifts the moon-eyes/smile around even though the robot is "happy".
    setBlinkMode(false);
    setIdleMode(false);
...
void faceNoEmo()
{
    ...
    // faceHappy() switches idle drift off; bring it back here, but only
    // if the robot is actually stopped -- same rule faceSetMotion() uses
    // for FACE_MOTION_STOP, so eyes don't start drifting mid-drive.
    if (currentMotion == FACE_MOTION_STOP) {
